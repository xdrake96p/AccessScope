/**
 * Euristiche generali e app-specifiche di precisione.
 */
...
internal object PrecisionRulesApp {
    ...
    fun shouldSkipSilentDynamicContent(
        ...
        if (snapshots.any { PrecisionRulesPlatform.isMapSurface(it) && it.bounds.width() * it.bounds.height() > estimateViewport(snapshots).let { v -> v.width() * v.height() } * 0.25f }) {
            return true
        }
        return false
    }
